...
{
	if (!Controller) return; //컨트롤러가 있는지 확인하는 안전코드 

	const FVector2D MoveInput = value.Get<FVector2D>();

	if (!FMath::IsNearlyZero(MoveInput.X)) //float부동소수점 형태는 정수에 가까운 실수라서 0에 가까울때 0으로 인정하는 안전코드
	{
		AddMovementInput(GetActorForwardVector(), MoveInput.X);// 캐릭터 클래스 내에 이미 정의된 함수
	}
